...
- Which file do you update to make Ansible run the playbook on a specific machine? the hosts file, add the elk server 10.1.0.4 under the webservers 
   - How do I specify which machine to install the ELK server on versus which to install Filebeat on? Update the hosts file in /etc/anisble/hosts Add the private IP of the ELK virtual machine in the [ELK] group under the [webservers] group 
 ![TODO: hosts file](Ansible/hosts) 
